...
$catStmt = $pdo->query('SELECT id, name FROM blog_categories WHERE status = "1" ORDER BY name DESC');
$blogCategories = $catStmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if ($newImagePath) {
                $upd = $pdo->prepare('UPDATE blogs SET user_id = ?, category_id = ?, gallery_id = ?, image = ?, title = ?, slug = ?, description = ?, updated_at = NOW() WHERE id = ? LIMIT 1');
                $upd->execute([
                        $_SESSION['admin_id'],
                        $categoryId,
                        $galleryId,
                ...
                $upd = $pdo->prepare('UPDATE blogs SET user_id = ?, category_id = ?, gallery_id = ?, title = ?, slug = ?, description = ?, updated_at = NOW() WHERE id = ? LIMIT 1');
                $upd->execute([
                        $_SESSION['admin_id'],
                        $categoryId,
                        $galleryId,
