...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('SELECT id FROM blog_categories WHERE name = ? LIMIT 1');
        $stmt->execute([$name]);
        if ($stmt->fetch()) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare('INSERT INTO blog_categories (name, status, created_at, updated_at) VALUES (?, "1", NOW(), NOW())');
            $stmt->execute([$name]);
